## https://github.com/PMunch/nim-persistent-vector/blob/master/persvector.nim
...
func add*[T](vec: PersistentVector[T], elem: T): PersistentVector[T] =
  ...
  if vec.tail.len != WIDTH:
    deepCopy.deepCopy(result.tail, vec.tail)
...
func toPersistentVector*[T](s: openArray[T]): PersistentVector[T] =
